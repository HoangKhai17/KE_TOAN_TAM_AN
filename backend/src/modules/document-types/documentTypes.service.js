...
// Admin: toàn quyền. Staff: chỉ chứng từ của công ty MÌNH phụ trách. (giống locations)
async function assertCompanyAccess(companyId, user) {
  ...
  if (user && user.role !== 'admin' && c.assigned_staff_id !== user.id) {
    throw Object.assign(new Error('Bạn không có quyền truy cập chứng từ của công ty này'), { status: 403 })
  }
}

async function listDocumentTypes(companyId, user) {
  await assertCompanyAccess(companyId, user)
  const { rows } = await query(
    `SELECT * FROM company_document_types
      WHERE company_id = $1
...
async function updateDocumentType(companyId, id, data, user) {
  await assertCompanyAccess(companyId, user)
  const actorId = user.id
  const { rows: [existing] } = await query(
    'SELECT id FROM company_document_types WHERE id = $1 AND company_id = $2',
    [id, companyId]
  )
  if (!existing) throw Object.assign(new Error('Document type not found'), { status: 404 })
  ...
  params.push(id)
  const { rows: [row] } = await query(
    `UPDATE company_document_types SET ${updates.join(', ')} WHERE id = $${params.length} RETURNING *`,
    params
  )
  return toDto(row)
}

async function deleteDocumentType(companyId, id, user) {
  await assertCompanyAccess(companyId, user)
  const { rows: [row] } = await query(
    'SELECT id FROM company_document_types WHERE id = $1 AND company_id = $2',
    [id, companyId]
  )
  if (!row) throw Object.assign(new Error('Document type not found'), { status: 404 })
  await query('DELETE FROM company_document_types WHERE id = $1', [id])
}
