...
// Ô ngày "lai": vừa GÕ TAY được (dd/mm/yyyy), vừa bấm icon lịch để chọn.
// Hiển thị luôn theo dd/mm/yyyy; giá trị lưu vẫn là YYYY-MM-DD.
...
export default function DateBox({ value, onChange, className = '', placeholder = 'dd/mm/yyyy' }) {
  ...
  // Chốt giá trị khi rời ô / Enter. Gõ sai -> khôi phục giá trị cũ, không lưu.
  function commit(raw) {
    const parsed = parseDateInput(raw)
    if (parsed === null) { setText(iso ? fmtDate(iso) : ''); setInvalid(false); return }
    setInvalid(false)
  ...
  return (
    <div className={`${s.dateBox} ${s.dateBoxHybrid} ${className} ${invalid ? s.dateBoxError : ''}`}>
      <input
        type="text"
        inputMode="numeric"
        ...
        onChange={(e) => {
          ...
          setText(v)
          setInvalid(v.trim() !== '' && parseDateInput(v) === null)
        }}
