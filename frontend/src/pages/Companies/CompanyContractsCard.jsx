    ...
    <div className={s.hdldDdFilterSection}>
      <div className={s.hdldDdRangeGroup}>
        <div className={s.hdldDdRangeRow}><span className={s.hdldDdRangeLabel}>Từ ngày</span>
          <input type="date" className={s.hdldDdInput} value={from} onChange={(e) => { setFrom(e.target.value); apply(e.target.value, to) }} /></div>
        <div className={s.hdldDdRangeRow}><span className={s.hdldDdRangeLabel}>Đến ngày</span>
          <input type="date" className={s.hdldDdInput} value={to} onChange={(e) => { setTo(e.target.value); apply(from, e.target.value) }} /></div>
      </div>
...
function ContractFilesModal({ contract, canEdit, onClose, onChanged }) {
  ...
  const [files, setFiles] = useState([]); const [loading, setLoading] = useState(true); const [uploading, setUploading] = useState(false)
  const fileRef = useRef(null)
  const load = useCallback(async () => {
    setLoading(true)
    try { const f = await attachmentsApi.listFiles(ATTACH_MODULE, contract.id); setFiles(f); return f }
    catch { addToast('Không tải được danh sách file', 'error'); return null }
    finally { setLoading(false) }
  }, [contract.id, addToast])
  useEffect(() => { load() }, [load])
  async function onPick(e) {
    const file = e.target.files?.[0]; e.target.value = ''
    if (!file) return
    if (file.size > attachmentsApi.MAX_FILE_BYTES) { addToast(`File quá lớn (tối đa ${attachmentsApi.formatSize(attachmentsApi.MAX_FILE_BYTES)})`, 'error'); return }
    setUploading(true)
    try { await attachmentsApi.uploadFile(ATTACH_MODULE, contract.id, file); addToast('Đã tải file lên', 'success'); const f = await load(); onChanged?.(f?.length ?? 0) }
    catch (err) { addToast(err.response?.data?.error?.message ?? 'Tải file thất bại', 'error') } finally { setUploading(false) }
  }
  ...
  return (
    <Modal title={`File đính kèm — hợp đồng`} onClose={onClose}>
      {loading ? <div className={s.locFileEmpty}><Loader2 size={16} className={s.spin} /> Đang tải…</div>
        : files.length === 0 ? <div className={s.locFileEmpty}>Chưa có file đính kèm.</div>
        : <div className={s.locFileList}>
            {files.map((f) => (
              <div key={f.id} className={s.locFileRow}>
                ...
                <button className={s.locFileIconBtn} title="Tải xuống" onClick={() => attachmentsApi.downloadFile(f.id, f.fileName)}><Download size={14} /></button>
                {canEdit && <button className={`${s.locFileIconBtn} ${s.locFileIconDanger}`} title="Xoá" onClick={() => remove(f)}><Trash2 size={14} /></button>}
              </div>
            ))}
          </div>}
      {canEdit && (
