...
function NoteEditModal({ companyId, row, onSave, onClose }) {
  ...
  return (
    <Modal title={`Ghi chú — ${row.name}`} onClose={requestClose} wide>
...
export default function OriginalDocumentsSection({ companyId, canEdit = true }) {
  ...
  return (
    <div>
      ...
      {filterPopup && <ColumnFilterDropdown colKey={filterPopup.colKey} filterType="text" allRows={rows} getDisplayLabel={(row, key) => String(row[key] ?? '')} currentFilter={colFilters[filterPopup.colKey] ?? null} sortState={sortState} onSort={(col, dir) => { setSortState(dir ? { col, dir } : { col: null, dir: 'asc' }); setFilterPopup(null) }} onFilterChange={setColumnFilter} onClose={() => setFilterPopup(null)} style={{ '--cfd-top': `${filterPopup.top}px`, '--cfd-left': `${filterPopup.left}px` }} />}
