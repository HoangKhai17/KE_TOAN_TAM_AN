...
import {
  ...
  Pencil, Save, X, Loader2, Baseline, Highlighter, Maximize2, Minimize2, PaintBucket,
} from 'lucide-react'
...
function ImageNodeView({ node, selected }) {
  ...
  return (
    <NodeViewWrapper as="span" className={`pde-img ${selected ? 'pde-img-selected' : ''}`}>
      {url
        ? <img src={url} alt={node.attrs.alt || ''} draggable={false} />
        : <span className="pde-img-ph">{err ? '⚠ Lỗi tải ảnh' : 'Đang tải ảnh…'}</span>}
    </NodeViewWrapper>
  )
}
...
// ── Re-render toolbar khi con trỏ/định dạng đổi (để nút sáng đúng trạng thái) ──────
...
function TB({ onClick, active, disabled, title, children }) {
  return (
    <button type="button" className={`pde-btn ${active ? 'pde-active' : ''}`}
      onMouseDown={(e) => e.preventDefault()} onClick={onClick} disabled={disabled} title={title}>
      {children}
    </button>
  )
}
const Sep = () => <span className="pde-sep" />

// Bảng màu chọn nhanh (chữ / nền chữ / nền ô)
...
function ColorMenu({ editor, mode }) {
  ...
  return (
    <span className="pde-colorwrap" ref={ref}>
      <button type="button" className={`pde-btn ${current ? 'pde-active' : ''}`}
        onMouseDown={(e) => e.preventDefault()} onClick={() => setOpen((v) => !v)}
        title={meta.title}>
        <Icon size={15} />
        <span className="pde-colorbar" style={{ background: current || meta.def }} />
      </button>
      {open && (
        <div className="pde-colormenu" onMouseDown={(e) => e.preventDefault()}>
          <div className="pde-swatches">
            {swatches.map((c) => (
              <button key={c} type="button" className="pde-swatch" style={{ background: c }}
                title={c} onClick={() => apply(c)} />
            ))}
          </div>
          <div className="pde-colormenu-row">
            <label className="pde-colorpick" title="Tự chọn màu">
              <input type="color" defaultValue={current || meta.def}
                onChange={(e) => apply(e.target.value)} />
              Tự chọn…
            </label>
            <button type="button" className="pde-colorclear" onClick={clear}>Bỏ màu</button>
          </div>
        </div>
      )}
    </span>
...
// ── Thanh công cụ ─────────────────────────────────────────────────────────────
function Toolbar({ editor, companyId, onInsertImage, onOpenMarkdown }) {
  ...
  return (
    <div className="pde-toolbar">
      ...
      <Sep />
      <TB title="Chèn ảnh" onClick={onInsertImage}><ImagePlus size={15} /></TB>
    ...
    <Modal title="Dán nội dung Markdown" onClose={onClose} wide>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
          <button className="pde-btn" style={{ border: '1px solid var(--color-border)', height: 34, padding: '0 14px' }} onClick={onClose}>Huỷ</button>
          <button className="pde-btn pde-active" style={{ height: 34, padding: '0 16px' }}
...
export default function ProcessDocEditor({ companyId, process, canEdit, onSaved, onDirtyChange }) {
  ...
  const [full, setFull] = useState(false)         // chế độ toàn màn hình
  ...
  const editor = useEditor({
    ...
    extensions: [
      ...
      Placeholder.configure({ placeholder: 'Soạn nội dung quy trình… (có thể dán từ Word, Google Docs, hoặc dùng nút “Dán Markdown”)' }),
      Table.configure({ resizable: true }),
      TableRow, HeaderWithBg, CellWithBg,
    ],
    content: process.content || '',
    onUpdate: () => setDirtyBoth(true),
  })

  // Tải ảnh lên attachments rồi chèn node ảnh (dùng chung cho nút chèn / paste / kéo-thả)
  ...
  // Bắt paste/kéo-thả ảnh từ ngoài → upload thay vì nhúng base64
  useEffect(() => {
    if (!editor) return undefined
  ...
  // Bật/tắt chế độ sửa
  useEffect(() => { if (editor) editor.setEditable(editing) }, [editor, editing])
  ...
  const isEmpty = !process.content || !process.content.replace(/<[^>]*>/g, '').trim()

  const FullBtn = (
    <TB title={full ? 'Thu nhỏ' : 'Toàn màn hình'} onClick={() => setFull((v) => !v)}>
      {full ? <Minimize2 size={15} /> : <Maximize2 size={15} />}
    </TB>
  )

  return (
    <div className={`pde-wrap ${editing ? '' : 'pde-view'} ${full ? 'pde-full' : ''}`}>
      {/* Thanh trên: trạng thái + nút toàn màn hình + Chỉnh sửa / Huỷ · Lưu (luôn thấy) */}
      ...
      {showMarkdown && (
        <MarkdownModal onInsert={insertMarkdown} onClose={() => setShowMarkdown(false)} />
      )}
    </div>
  )
}
